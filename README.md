...
- **FantaBandeja in archivio**: la stagione è conclusa e la sezione è in sola consultazione. Restano disponibili **Classifica**, **Risultati** e il regolamento storico; **Partite**, nuove formazioni, nuovi calcoli e notifiche fantasy sono sospesi. Nessun round o punteggio storico viene eliminato o riscritto.
- Registrazione e accesso con email e password; dal menu account ogni giocatore può cambiare soltanto il nome visibile, aggiungere o rimuovere una foto profilo e scegliere le sette categorie di notifica ancora operative: sveglia del lunedì, nuovi slot, formazione completa, sostituzioni, campo da prenotare, partita a 24 ore e partita a 2 ore. Le preferenze dei giudizi post-partita e di FantaBandeja restano conservate nei profili ma sono nascoste finché le funzioni sono sospese. Il menu si chiude al primo clic o tocco esterno e con il tasto `Esc`, senza interrompere le interazioni al suo interno. Email e password non sono modificabili dalla sezione profilo. Il nome resta l'unico mostrato nell'interfaccia e non viene mai ricavato dall'indirizzo email; un cambio nome che contiene la sottostringa `Evi`, senza distinzione tra maiuscole e minuscole, viene rifiutato mostrando il bordo rosso e l'errore direttamente sotto il campo.
...
- Il motore storico di **FantaBandeja** resta nel codice per una possibile riapertura: conserva formazioni, capitano, modelli `ratings-v1`, `mvp-v2` e `feedback-v3`, dettaglio auditabile del punteggio, classifica e bonus presenza già materializzati. In modalità archivio il client non salva entry, il repository non riconcilia i round, il notifier non li crea o calcola e le Security Rules rifiutano ogni nuova scrittura fantasy.
- Bonus presenza storico FantaBandeja: nei round già calcolati ogni titolare conserva `2` punti nella classifica generale per la partita giocata e chi condivideva il giudizio medio migliore ne conserva `3`. La modalità archivio non modifica questi dati.
...
- **Giudizi post partita sospesi**: non vengono più mostrate schede, inviati promemoria o accettati nuovi giudizi. Codice, preferenze e storico restano conservati; gli aggregati già presenti possono ancora essere letti nelle schede storiche, ma Firestore rifiuta nuove risposte anche da client non aggiornati.
...
- Notifiche Web Push opzionali per i nuovi slot disponibili e, per chi è tra i quattro titolari, l’avviso che la formazione è completa, la convocazione personale quando riceve il posto tramite sostituzione, un secondo promemoria a una settimana dalla partita se il campo è ancora da prenotare e i reminder a 24 ore e 2 ore dalla partita prenotata. Le categorie post-partita e FantaBandeja sono sospese e filtrate centralmente dal notifier. Gli avvisi raggruppati identificano il sondaggio tramite il suo intervallo settimanale anche per i documenti creati prima dell’introduzione del nome automatico. Gli avvisi di prenotazione non partono se il campo è già confermato; gli slot inseriti entro 10 minuti l’uno dall’altro vengono riuniti in un solo avviso con identità stabile e, dopo un’ora dall’ultima aggiunta, non possono più essere annunciati come nuovi.
