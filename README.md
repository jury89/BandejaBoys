...
Durante il gioco la vista **Turno in corso** riunisce timer, risultati dei campi e conferma del turno; **Classifica** e **Dettagli** separano consultazione e impostazioni. Al passaggio successivo la pagina torna ai comandi del nuovo turno, senza cercarli tra iscritti e regolamento. Nei gironi a tempo, **Concludi turno** ferma anticipatamente il timer per tutti dopo conferma dell’organizzatore/admin: i punteggi restano modificabili e servono comunque tutti i risultati prima di proseguire. La simulazione privata segue lo stesso flusso.

Nel menu account di tutti i membri compare **Tornei** (`#tornei`). Chiunque può creare un torneo: salva una bozza privata, sceglie data/ora italiana, circolo, campi disponibili e formula, poi pubblica e condivide il link. La gestione spetta al creatore e a Jury; le bozze sono leggibili solo da loro. Gli altri membri vedono i tornei pubblicati nell’elenco e possono iscriversi fino a **un’ora prima dell’inizio**; nelle coppie scelte entrambi devono confermare il compagno. Sono disponibili **Americano**, **Mexicano**, **girone all’italiana** ed **eliminazione diretta con finalina**, con descrizioni e criteri di classifica nella pagina.

Alla chiusura delle iscrizioni l’organizzatore (o Jury) prepara il tabellone. Da quel momento si può giocare anche prima dell’orario previsto: nei gironi a tempo organizzatore e Jury avviano il timer quando tutti sono pronti. I risultati sono disponibili dopo l’avvio effettivo del timer (dopo il sorteggio per le formule senza timer), senza aspettare l’ora programmata. Organizzatore e Jury possono inserire/correggere qualsiasi risultato; i giocatori coinvolti solo quello della propria partita, salvo modalità solo organizzatore. Confermando ogni turno si bloccano i relativi risultati; nell’ultimo turno viene assegnato il podio. Nessuna prenotazione automatica e nessuna modifica a Fanta, giudizi o storico ordinario. I pari merito completi sono espliciti, non risolti per ordine alfabetico.

In **Dettagli → Gestione torneo**, creatore e Jury possono correggere le coppie fisse sorteggiate dal computer prima del primo risultato o timer. Possono anche modificare gli avversari nei turni ancora da giocare: la scelta di una squadra o di un giocatore scambia automaticamente i due posti nello stesso turno. Il girone conserva tutte le sfide una volta sola; le partite con risultati e i turni già avviati non cambiano. Le correzioni funzionano anche nella simulazione privata e non modificano le coppie scelte reciprocamente dai giocatori.
...
Il **girone a tempo** si adatta a numero di iscritti, campi prenotati e durata totale disponibile: nessun preset obbligatorio. Imposta capienza (6–32, pari), campi (1–8), tempo totale (15–720 minuti), riscaldamento e cambi; l’app propone turni, durata delle partite, riposi e margine. Con 10 giocatori, 2 campi e 90 minuti propone 5 turni da 15 minuti; con 12 giocatori sugli stessi campi propone 8 turni da 8 minuti. Tutte le coppie fisse si incontrano e giocano lo stesso numero di partite. Se non restano almeno 5 minuti per incontro l’app indica il tempo minimo necessario, senza eliminare partite o persone. Al sorteggio il programma viene ricalcolato sugli iscritti effettivi e congelato; un numero dispari impedisce il via. I tornei già avviati e quelli legacy a durata fissa non cambiano automaticamente.

**Massimo partecipanti** è un campo numerico scrivibile e comprende membri e ospiti; raggiunto il limite le iscrizioni si fermano, senza obbligo di riempire tutti i posti. Restano i limiti della formula (massimo 32) e non si può scendere sotto gli iscritti esistenti. **Modifica torneo** è disponibile per ogni formula: il creatore modifica i propri tornei prima del cutoff, solo Jury può modificare anche quelli degli altri e intervenire a iscrizioni chiuse prima del sorteggio. Dal primo iscritto formula, coppie e punti totali per incontro sono protetti; nel girone si può ancora cambiare **Durata delle partite** tra set e partite a tempo, conservando iscritti e compagni. Cambiare data aggiorna il cutoff e può riaprire le iscrizioni. Dopo il sorteggio (o su tornei conclusi/annullati), Jury può correggere soltanto nome e circolo: tabellone, iscrizioni e risultati restano intatti.

Solo Jury vede **Tornei → Simulazione privata**: esempio 10 giocatori/2 campi/90 minuti con dati fittizi, oppure **Prova in privato** per copiare le impostazioni di un torneo. Può saltare al via, compilare risultati, far scadere i timer e provare il podio. La prova rimane nel browser di quel dispositivo e non scrive su Firestore, non manda notifiche e non modifica tornei/Fanta/statistiche. Non è pubblicabile; gli altri account non possono aprirla nemmeno dal link. Istruzioni complete in [docs/tournaments.md](docs/tournaments.md).
...
Per provare senza cambiare la preferenza: [nuova interfaccia](https://bandeja-boys.web.app/?ux=nuova) oppure [classica](https://bandeja-boys.web.app/?ux=classica). Il parametro valido prevale sul profilo; salvando il profilo diventa permanente la scelta mostrata nel modulo e il parametro viene rimosso. Annullando non cambia nulla. L’anteprima online usa gli stessi dati reali: iscrizioni e modifiche non sono simulate.

Dettagli, perimetro del rilascio e collaudi in [docs/restyling.md](docs/restyling.md).
...
- **Interfaccia Clubhouse mobile-first**: navigazione diretta Bacheca / Partite / Fanta / Statistiche, in basso sul telefono e in alto sul desktop. Gli slot mostrano un’unica rosa compatta con foto o iniziali centrate e lo stato personale separato dalla prenotazione; i filtri includono posti liberi e proprie iscrizioni (anche riserve). Le azioni gestionali hanno etichette nel menu sovrapposto “Altre azioni”. La riserva può scegliere **Passa a titolare** quando c’è posto, senza uscire e rientrare; ospiti e riserve hanno righe leggibili. Partite personali prossime/giocate sono separate, lo storico si mostra a blocchi e il selettore delle statistiche permette la ricerca per nome. Logo e palette originali restano invariati. Dettagli in [docs/restyling.md](docs/restyling.md).
- **Navigazione e stagioni FantaBandeja**: una plancia interna separa **Partite**, **Classifica** e **Risultati** e lascia visibile un solo contenuto alla volta. **Partite** contiene soltanto le formazioni ancora aperte; appena scatta il blocco, il round passa in cima a **Risultati** come “in calcolo”, prima dell’archivio concluso. La stagione **Estate 2026** comprende tutte le partite fino alle 23:59 del 27 settembre nel fuso `Europe/Rome`; allo scoccare della mezzanotte parte automaticamente **Inverno 2026/27** con classifica e punti separati. Fino alla chiusura si vedono soltanto l’estiva e il suo countdown, senza un selettore anticipato; dal 28 settembre compaiono entrambe le stagioni, con l’invernale selezionata per default e l’estiva consultabile in sola lettura. L’app assegna ogni round dalla data effettiva della partita, quindi un calcolo completato dopo la mezzanotte non sposta il match nella stagione successiva e non richiede alcuna migrazione dello storico. Lo storico mostra quattro round alla volta, apre soltanto il più recente e lascia collassati gli altri, con riepilogo di vincitore e punteggio personale prima dei dettagli; **Mostra altri** carica progressivamente il resto dell’archivio. Le tab seguono il pattern ARIA completo anche da tastiera, gli errori permettono di riprovare senza ricaricare tutta l’app e il regolamento resta disponibile dal pulsante “Come si gioca”.
...
- Punti in campo FantaBandeja: **Estate 2026** conserva `2` punti per titolare, oppure `3` per chi condivide il giudizio medio migliore (MVP nei modelli storici). **Inverno 2026/27**, dalle 00:00 italiane del 28 settembre, assegna invece `5/3/1/0` in base al totale individuale già salvato (`media giudizi + bilancio set + bonus differenza game`), senza aggiungere punti presenza. A pari totale si condividono posizione e premio, saltando le posizioni occupate: due primi prendono `5/5/1/0`. Il primo è il **Miglior giocatore della partita**, distinto dal **Miglior giudizio medio** che continua a determinare il bonus capitano. Formazioni, moltiplicatori, spareggi e vittorie Fanta restano invariati. Decide la data d’inizio della partita, non quella del calcolo: lo storico estivo non cambia neppure con ricalcoli tardivi. Piazzamenti e punti in campo sono visibili nei risultati e nel dettaglio della generale. Non servono migrazioni, nuove query, modifiche alle Rules o allo scheduler; rilascio Hosting.
...
L'app è progettata per contenere i consumi nelle franchigie gratuite. Il 9 settembre 2026 il proprietario ha attivato Firebase Blaze in produzione dopo l'esaurimento della quota Spark: i consumi oltre le franchigie possono quindi essere addebitati. Le foto profilo non usano Firebase Storage: vengono ritagliate e compresse nel browser a `160×160` pixel e salvate come piccolo Data URL nel documento Firestore del proprietario, con un limite di 100.000 caratteri applicato anche dalle Security Rules. La franchigia Firestore di 50.000 letture al giorno va monitorata: scansioni frequenti dello storico possono esaurirla anche con pochi utenti. La quota giornaliera si ripristina intorno a mezzanotte Pacific Time; un'ottimizzazione non restituisce la quota già consumata. Lo scheduler usa 144 delle 100.000 richieste giornaliere incluse nel piano gratuito Cloudflare Workers. Riferimenti: [piani Firebase](https://firebase.google.com/docs/projects/billing/firebase-pricing-plans), [quote Firestore](https://firebase.google.com/docs/firestore/quotas), [Firebase Hosting](https://firebase.google.com/docs/hosting/quickstart) e [limiti Cloudflare Workers](https://developers.cloudflare.com/workers/platform/limits/).
...
Per il collaudo esplorativo del restyling usare una sessione browser separata, senza azzerare quella dell’utente. Verificare anche il menu account su schermi bassi/orizzontali, la geometria di avatar e interruttori, la navigazione da tastiera e la persistenza di profilo, iscrizioni, referti e formazione Fanta dopo un ricaricamento. I dettagli e i limiti del collaudo sono in [docs/restyling.md](docs/restyling.md).
...
Il notifier legge risposte, aggregati e referti soltanto per le partite ancora utili a notifiche o calcolo; non ricarica le formazioni dei round già calcolati. I risultati/annullamenti sono notificabili per sette giorni dal calcolo, senza ricontrollare per sempre le vecchie ricevute. Un round annullato per referto mancante continua invece a essere controllato: se arriva il referto tardivo, recupera i dati necessari e viene ricalcolato normalmente. Non vengono cancellati dati storici. Le query usano il solo indice standard su `slotId`, in gruppi di massimo 30 valori, senza migrazioni o nuovi indici composti. Le push manuali leggono soltanto profilo e dispositivi del destinatario e non eseguono calcoli Fanta; il catalogo motivazionale si carica solo nella finestra del lunedì. Ogni run riporta una stima minima delle letture per raccolta (escluse letture delle Security Rules e degli indici), da confrontare con la console Firebase per verificare il consumo reale.
