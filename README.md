...
- Modifica di data e ora degli slot già pubblicati senza perdere adesioni, riserve o prenotazione. Quando tutti gli slot vengono spostati nella stessa nuova settimana, anche intervallo e titolo del sondaggio si riallineano automaticamente; durante uno spostamento parziale su settimane diverse restano invariati fino all’ultima modifica coerente. Ogni slot può anche essere eliminato, finché ne resta almeno uno nel sondaggio: la conferma chiarisce che adesioni e riserve verranno rimosse e, per un campo prenotato, ricorda di annullare anche presso l’Oasi Boschetto.
...
- **FantaBandeja**, raggiungibile dal menu account: per ogni partita futura prenotata con quattro titolari registrati, chi non è tra quei quattro schiera due giocatori e assegna a uno la fascia. Entrambi i riquadri della coppia scelta restano evidenziati in lime, anche dopo il secondo tocco. Il pulsante “Come si gioca” apre il regolamento completo senza lasciare la pagina. La scelta resta privata e modificabile fino all’orario d’inizio, poi viene bloccata e resa visibile insieme alle altre. Se prima del via la partita torna temporaneamente incompleta, il runner sospende e nasconde il round, senza notifiche o falso annullamento nello storico; appena si completa, ripristina la rosa corrente e rende obsoleta la giocata soltanto se è cambiato almeno un titolare. Il punteggio di ogni giocatore parte dalla media delle pagelle della partita, usa `6` d’ufficio se sono arrivate meno di due valutazioni, aggiunge `+1,5` per più set vinti che persi (`-0,5` nel caso opposto) e `+0,5` a chi condivide la miglior differenza game positiva. Nei risultati di ogni round, la riga di ciascun giocatore si può espandere per mostrare l’equazione completa e il dettaglio di voto base, bilancio set e differenza game; il dettaglio funziona anche per tutto lo storico già calcolato perché riusa i componenti materializzati nei `playerScores`. Il capitano vale `×1,5` e guadagna altri `+2` se è MVP della pagella. Il round assegna `5`, `3` e `1` punti ai primi tre; a parità usa nell’ordine pagella del capitano e somma delle due pagelle, poi conserva l’ex aequo. La generale ordina per punti campionato, vittorie e punti fantasy grezzi; ogni nome è espandibile e mostra, round per round, i punti da piazzamento e i bonus presenza/MVP che compongono il totale. Da 24 ore dopo la fine il calcolo parte appena sono presenti il referto e almeno due valutazioni per ciascun giocatore; a 48 ore il round si chiude comunque, usando `6` d’ufficio per le pagelle incomplete oppure annullandosi se manca il referto. Se il referto viene inserito più tardi, un round annullato esclusivamente per quel motivo viene ricalcolato automaticamente e genera il normale risultato; gli altri motivi di annullamento restano definitivi. Una sostituzione invalida la vecchia giocata e chiede di ricomporla; una modifica di data, ora o durata aggiorna invece le scadenze del round senza invalidare la coppia finché i quattro titolari restano gli stessi. Ospiti e titolari del match non possono partecipare al relativo round.
- Bonus presenza FantaBandeja: ogni titolare riceve automaticamente `2` punti nella classifica generale per la partita giocata; chi risulta MVP per media pagella ne riceve `3`. Il bonus viene derivato dai `playerScores` salvati nel round e vale anche retroattivamente per tutte le giornate già calcolate, senza modificare voti o referti storici.
...
- Pagelle post partita: appena termina un campo prenotato, ogni titolare registrato trova nell’app una scheda per assegnare da 1 a 10 agli altri compagni registrati e diventa destinatario della relativa notifica. Sul piano gratuito, la push viene consegnata dal primo controllo dello scheduler successivo alla fine, normalmente entro 10 minuti; gli eventuali ospiti senza account non vengono proposti. Il salvataggio conserva partita, autore, destinatario e timestamp e aggiorna anche somma e conteggio del destinatario nello stesso batch atomico compatibile con le interruzioni temporanee di rete della PWA; chi chiude la scheda la elimina definitivamente soltanto per sé. Nello storico di **I miei match**, ogni partita per cui il giocatore ha ricevuto almeno un voto mostra la relativa media su 10 nella mini scheda, arrotondata al mezzo punto più vicino; **Gli altri match** legge soltanto l’aggregato anonimo.
