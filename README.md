...
- Pagina **Gli altri match**, anch’essa nel menu account: elenca a ritroso soltanto partite complete, prenotate e concluse in cui il giocatore corrente non era tra i quattro titolari. Ogni scheda mostra i quattro partecipanti e, quando disponibili, il vincitore o i vincitori MVP con il relativo numero di preferenze. Quando esiste un referto, coppie e punteggi compaiono nello stesso tabellone per squadre di **I miei match**; in caso contrario la scheda dichiara esplicitamente che il referto non è stato aggiunto. Gli aggregati MVP non contengono l’identità dei votanti; le singole scelte restano riservate al loro autore.
...
- **FantaBandeja**, raggiungibile dal menu account: per ogni partita futura prenotata con quattro titolari registrati, chi non è tra quei quattro schiera due giocatori e assegna a uno la fascia. Entrambi i riquadri della coppia scelta restano evidenziati in lime, anche dopo il secondo tocco. Il pulsante “Come si gioca” apre il regolamento completo senza lasciare la pagina. La scelta resta privata e modificabile fino all’orario d’inizio, poi viene bloccata e resa visibile insieme alle altre. Se prima del via la partita torna temporaneamente incompleta, il runner sospende e nasconde il round, senza notifiche o falso annullamento nello storico; appena si completa, ripristina la rosa corrente e rende obsoleta la giocata soltanto se è cambiato almeno un titolare. Il punteggio di ogni giocatore parte da `6`, aggiunge `+1,5` per più set vinti che persi (`-0,5` nel caso opposto), `+0,5` a chi condivide la miglior differenza game positiva e `+1` a ogni co-MVP con il maggior numero di preferenze. Nei risultati di ogni round, la riga di ciascun giocatore si può espandere per mostrare l’equazione completa, il bilancio set, la differenza game e le preferenze MVP. Il capitano vale `×1,5` e guadagna altri `+2` se è MVP. Il round assegna `5`, `3` e `1` punti ai primi tre; a parità usa nell’ordine il punteggio del capitano e la somma della coppia, poi conserva l’ex aequo. La generale ordina per punti campionato, vittorie e punti fantasy grezzi; ogni nome è espandibile e mostra, round per round, i punti da piazzamento e i bonus presenza/MVP che compongono il totale. Da 24 ore dopo la fine il calcolo parte appena esistono il referto e tutte e quattro le risposte MVP, comprese eventuali chiusure senza scelta; a 48 ore il round si chiude comunque con le preferenze disponibili oppure viene annullato se manca il referto. Se il referto viene inserito più tardi, un round annullato esclusivamente per quel motivo viene ricalcolato automaticamente e genera il normale risultato; gli altri motivi di annullamento restano definitivi. Una sostituzione invalida la vecchia giocata e chiede di ricomporla; una modifica di data, ora o durata aggiorna invece le scadenze del round senza invalidare la coppia finché i quattro titolari restano gli stessi. Ospiti e titolari del match non possono partecipare al relativo round. I round già calcolati conservano il vecchio dettaglio a pagelle per non riscrivere la storia.
- Bonus presenza FantaBandeja: ogni titolare riceve automaticamente `2` punti nella classifica generale per la partita giocata; ogni giocatore che condivide il maggior numero di preferenze MVP ne riceve `3`. Il bonus viene derivato dai `playerScores` salvati nel round e continua a valere anche sui round storici già calcolati, senza modificare voti o referti precedenti.
...
- Scelta MVP post partita: appena termina un campo prenotato, ogni titolare registrato trova nell’app una scheda one-shot per scegliere un solo MVP tra gli altri compagni registrati e diventa destinatario della relativa notifica. Sul piano gratuito, la push viene consegnata dal primo controllo dello scheduler successivo alla fine, normalmente entro 10 minuti; gli eventuali ospiti senza account non vengono proposti. Il salvataggio conserva partita, autore, giocatore scelto e timestamp e incrementa l’aggregato del candidato nello stesso batch atomico, compatibile con le interruzioni temporanee di rete della PWA; chi chiude la scheda la elimina definitivamente soltanto per sé. Le richieste non completate scadono dopo sette giorni. Nello storico di **I miei match** e **Gli altri match** compaiono vincitore o vincitori MVP e numero di preferenze, senza esporre l’identità dei votanti.
