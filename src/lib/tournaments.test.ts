...
describe('tournament domain', () => {
  it('creates private drafts only for the administrator and locks configuration on publish', () => {
    expect(() => makeTournament('qa', input, 'someone', now)).toThrow(/amministratore/)
    const draft = makeTournament('qa', input, admin, now)
    expect(draft).toMatchObject({ published: false, status: 'draft', registrations: {}, matches: {} })
    expect(editTournament(draft, { ...input, title: 'Nuovo nome' }, admin, now).title).toBe('Nuovo nome')
    const published = publishTournament(draft, admin, now)
...
describe('local tournament persistence', () => {
  ...
  it('keeps drafts private, publishes live updates, and respects permissions and score revisions', async () => {
    const repo = localTournamentRepository(), organizer = user(admin)
    const visible = vi.fn(), privateList = vi.fn(), failed = vi.fn()
