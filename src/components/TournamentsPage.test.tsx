...
describe('TournamentsPage', () => {
  ...
  it('lets the organizer correct computer-drawn pairs before scores and keeps match positions', async () => {
    const t = startTournament(fixture({ startsAt: now, format: 'round-robin', pairing: 'random-fixed' }, 8), admin.id, 42, now)
    seed(t); const u = userEvent.setup(); show()
    await u.click(screen.getByRole('button', { name: 'Dettagli' }))
    await u.click(screen.getByRole('button', { name: 'Modifica coppie sorteggiate' }))
    const modal = screen.getByRole('dialog')
    const first = t.teams[0].playerIds[0], second = t.teams[1].playerIds[0]
    const selectors = within(modal).getAllByLabelText('Giocatore 1')
    await u.selectOptions(selectors[0], second)
    await u.selectOptions(selectors[1], first)
    await u.click(within(modal).getByRole('button', { name: 'Salva coppie' }))
    expect(await screen.findByText('Coppie aggiornate nel tabellone.')).toBeInTheDocument()
    const saved = JSON.parse(localStorage.getItem('bandeja-tournaments-v1')!).tournaments[0] as Tournament
    expect(saved.teams[0].playerIds[0]).toBe(second)
    expect(saved.teams[1].playerIds[0]).toBe(first)
    expect(saved.matches['r1-m1'].court).toBe(t.matches['r1-m1'].court)
  })
  it('lets the organizer swap opponents in an unplayed knockout round', async () => {
    const t = startTournament(fixture({ startsAt: now, format: 'knockout', pairing: 'random-fixed' }, 8), admin.id, 42, now)
    seed(t); const u = userEvent.setup(); show()
    await u.click(screen.getByRole('button', { name: 'Dettagli' }))
    await u.click(screen.getByRole('button', { name: 'Modifica turni e avversari' }))
    const modal = screen.getByRole('dialog')
    const otherOpponent = t.matches['r1-m2'].teamB.id
    await u.selectOptions(within(modal).getAllByLabelText('Squadra 2')[0], otherOpponent)
    await u.click(within(modal).getByRole('button', { name: 'Salva turni' }))
    expect(await screen.findByText('Avversari e turni aggiornati.')).toBeInTheDocument()
    const saved = JSON.parse(localStorage.getItem('bandeja-tournaments-v1')!).tournaments[0] as Tournament
    expect(saved.matches['r1-m1'].teamB.id).toBe(otherOpponent)
    expect(saved.matches['r1-m2'].teamB.id).toBe(t.matches['r1-m1'].teamB.id)
    expect(saved.matches['r1-m1'].court).toBe(t.matches['r1-m1'].court)
  })
  it('hides draw editing from spectators and after results have been entered', async () => {
    const t = startTournament(fixture({ startsAt: now, format: 'round-robin', pairing: 'random-fixed' }, 8), admin.id, 42, now)
    seed(t); const spectator = show(members[7])
    await userEvent.setup().click(screen.getByRole('button', { name: 'Dettagli' }))
    expect(screen.queryByRole('button', { name: 'Modifica coppie sorteggiate' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Modifica turni e avversari' })).not.toBeInTheDocument()
    spectator.unmount()
    const scores = Object.values(t.matches).filter(match => match.round === 1)
      .map(match => makeTournamentScore(t, match.id, 6, 2, admin.id, undefined, 0, now))
    seed(t, scores); show()
    await userEvent.setup().click(screen.getByRole('button', { name: 'Dettagli' }))
    expect(screen.queryByRole('button', { name: 'Modifica coppie sorteggiate' })).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Modifica turni e avversari' })).toBeInTheDocument()
  })
