...
export function TournamentSimulationTools({ session, tournament: t, now, busy, onAction }: {
  ...
  return <section className="tournament-panel tournament-simulation" aria-label="Strumenti della simulazione">
    ...
    <div className="tournament-actions">
      ...
      {(t.status === 'open' || t.status === 'running') && now < t.startsAt && <button className="button" disabled={busy} onClick={() => action(() => session.jump('start'))}>Salta all’inizio del torneo</button>}
      {t.status === 'running' && now >= t.startsAt && (!timed || clock.state !== 'waiting') && <button className="button" disabled={busy} onClick={() => action(() => session.fillScores())}>Compila risultati di prova</button>}
