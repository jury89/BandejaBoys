...
- Optional `totalMinutes` enables adaptive full round robin: budget minus warmup/changeovers determines an integer match duration (minimum 5 minutes). Pack every pair exactly once into collision-free rounds bounded by courts; use actual entrants at draw, then freeze. Null/absent budget preserves legacy manual timing. Only creator/admin may change published capacity/courts/budget before the one-hour cutoff, never below existing attendance; no automatic removal or format switch.
