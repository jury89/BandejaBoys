...
- Tournament edits preserve ownership, registrations (including guests) and draws. Creator may edit draft/open tournaments before cutoff; only configured admin may edit other creators' tournaments or undrawn open settings after cutoff. After draw/completion/cancellation, only admin metadata (title/venue) edits are allowed. Once anyone joins, format/pairing/scoring are locked; capacity can never fall below attendance. A changed start must be over one hour in the future and updates cutoff; unchanged dates may be retained by admin after cutoff. Enforce in pure domain and Rules, not only UI.
