...
- Creator/admin can correct computer-drawn fixed teams before the first score/timer, and opponents in unplayed rounds. Preserve entrants, match IDs, courts, round-robin opponent coverage, Americano teammate frequency and knockout qualifiers. Draw edits read scores in the same transaction and increment `drawRevision`; old scores and started rounds remain immutable.
...
- Timed round robin uses one persisted start timestamp per round, concurrent courts, integer completed-game scores (draws allowed), and 3/1/0 standings. Never write a ticking timer to Firestore. Creator/admin may explicitly stop a running timer with immutable `roundEndedAt` (confirmation in UI). Advance only after expiry or explicit stop AND all results; reset both timestamps for the next manual start. Never restart a stopped timer or freeze scores merely by stopping it. Missing timing fields mean legacy standard scoring.
- Once the draw is ready, scheduled `startsAt` is informational for play: creator/admin can start the timer early, and authorized scores/round advancement follow the actual timer, not the scheduled start. Preserve the one-hour registration/draw cutoff, timestamp freshness, no-restart rule and all score/turn permissions. Standard matches can score after the draw. Apply equally to real and private rehearsal flows.
...
- External tournament entrants use `guest:<UUID>` and `isGuest: true`, count towards capacity and never create accounts. Only creator/admin add, remove or change their partner before the one-hour cutoff. Creator/admin may correct an existing guest's name at any time, including after the draw or completion, without changing identity, order, partners, draw or results. Chosen member/guest pairs still require reciprocal choices.
