...
Un referto usa l’identificatore deterministico `pollId__slotId` e fotografa i quattro titolari, i nomi, l’autore originario e l’ultima modifica. Per ciascuno dei massimo cinque set salva due coppie da due giocatori e i rispettivi punteggi. `getMatchPairings` genera esclusivamente le tre partizioni possibili dei quattro titolari e `makeMatchReport` ricostruisce la coppia avversaria dai partecipanti, quindi l’interfaccia e il repository non possono produrre duplicati o lasciare un giocatore fuori dal set. Il client ascolta `matchReports` con `array-contains` sul proprio UID e collega il documento alla partita usando la stessa coppia stabile; non serve un indice composto.
...
Dieci minuti dopo la fine di una partita prenotata con quattro titolari, ciascun titolare vede una pagella one-shot per gli altri tre. La scadenza è derivata da `startsAt + durationMinutes + 10 minuti` usando sempre il fuso `Europe/Rome`, anche nel runner GitHub configurato in UTC. Un deep link della notifica dà precedenza alla partita indicata; aprendo normalmente l’app dopo la scadenza viene proposta comunque la pagella più vecchia ancora in sospeso. La chiusura crea una risposta `dismissed`; il salvataggio crea una risposta `submitted` e i relativi voti in un unico write batch atomico. Non viene eseguita una lettura transazionale preliminare: le Security Rules `create`-only impediscono comunque duplicati e il batch, a differenza di una transazione client, resta utilizzabile quando la PWA attraversa un’interruzione temporanea di rete. Entrambi gli esiti impediscono per sempre che la stessa scheda venga riproposta allo stesso giocatore. Il client ascolta inoltre i record `matchRatings` in cui l’utente corrente è il destinatario: `getPlayerMatches` raggruppa quelli con la stessa coppia `pollId`/`slotId` e allega media aritmetica e numero di voti alla sola vista derivata, senza salvare aggregati duplicati. La mini scheda di **I miei match** conserva questo valore preciso nei dati ma lo presenta arrotondato con `Math.round(media * 2) / 2`, quindi a scatti di mezzo punto e senza decimale per i risultati interi.
...
Nel profilo, otto interruttori indipendenti controllano **motivazione del lunedì**, **nuovi slot**, **formazione completa**, **sostituzioni**, **campo da prenotare a 7 giorni**, **reminder 24 ore**, **reminder 2 ore** e **pagelle**. Le scelte sono salvate in `users/{uid}.notificationPreferences`, quindi valgono su tutti i dispositivi dello stesso account; l’assenza del campo o della preferenza `starterSubstitution`, tipica dei profili creati prima della relativa funzione, viene normalizzata alla categoria attiva. Il runner applica la preferenza dopo aver derivato i destinatari reali dal sondaggio e prima di creare la ricevuta di consegna. Le notifiche manuali operative di tipo `test` ignorano intenzionalmente questi interruttori, così resta possibile collaudare un dispositivo o inviare un messaggio richiesto esplicitamente.
...
I referti sono invece documenti condivisi e modificabili nella raccolta `matchReports`. Il primo salvataggio e ogni modifica usano `runTransaction`: la creazione conserva autore e timestamp iniziali, mentre gli aggiornamenti possono cambiare soltanto set, ultimo autore e ultimo timestamp. Le Security Rules richiedono quattro partecipanti distinti, da uno a cinque set senza pareggi e quattro giocatori distinti in ogni set; l’UID autenticato deve appartenere alla formazione. Lettura e aggiornamento sono quindi limitati ai titolari registrati, mentre la cancellazione è vietata.
