...
Ogni slot espone un’azione calendario anche nei sondaggi archiviati. Il client genera localmente un file iCalendar `.ics` con fuso `Europe/Rome`, durata, Oasi Boschetto e stato `TENTATIVE` o `CONFIRMED`; nessun dato viene inviato a un servizio calendario esterno. Modifica, eliminazione ed esportazione sono raccolte in una barra di pulsanti a icona con nomi accessibili.
