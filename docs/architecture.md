...
La bacheca mobile espone inoltre un pull-to-refresh esplicito. Il listener touch si arma soltanto quando `scrollY` è zero, riconosce un trascinamento prevalentemente verticale verso il basso e applica resistenza alla distanza visuale; oltre la soglia, il rilascio usa `location.replace` con il parametro univoco `_pullRefresh`. La navigazione conserva query e hash correnti, evita una voce aggiuntiva nella cronologia e forza il recupero dell’HTML e dei listener Firestore più recenti. Il gesto non viene montato nella sezione “I miei match” e si disarma se è aperto un modal, così non interferisce con form, calendari o gesture di navigazione.
